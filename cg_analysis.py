...
def create_heatmap(
    ...
    normalize_rows : bool, optional
        Whether to normalize each row (divide each value by the row total)
    ...
    pivot_df = filtered_df.pivot_table(
        index=row_col, columns=col_col, values=value_col, aggfunc="count", fill_value=0
    )
    ...
    for row_idx in pivot_df.index:
        for col_idx in pivot_df.columns:
            ...
            # Skip very small effect sizes (meaningful difference threshold)
            if effect_size < min_effect_size_prop:
                continue
    ...
    if len(working_pivot.columns) > max_categories:
        ...
        if other_columns:
            ...
            other_weighted_avg = sum(weighted_avg_by_entity2[col] for col in other_columns)
        ...
        for row_idx in working_pivot.index:
            for col_idx in top_columns:
                if row_idx in working_significance_df.index and col_idx in working_significance_df.columns:
                    new_significance_df.loc[row_idx, col_idx] = working_significance_df.loc[row_idx, col_idx]
            ...
            for row_idx in working_pivot.index:
                if row_idx in working_significance_df.index:
                    ...
                    for col in other_columns:
                        if col in working_significance_df.columns and working_significance_df.loc[row_idx, col]:
    ...
    if len(working_pivot.index) > max_categories:
        ...
        for row_idx in top_rows:
            for col_idx in working_pivot.columns:
                if row_idx in working_significance_df.index and col_idx in working_significance_df.columns:
                    new_row_significance_df.loc[row_idx, col_idx] = working_significance_df.loc[row_idx, col_idx]
            ...
            for col_idx in working_pivot.columns:
                if col_idx in working_significance_df.columns:
                    ...
                    for row in other_rows:
                        if row in working_significance_df.index and working_significance_df.loc[row, col_idx]:
    ...
    # Create normalized version for visualization if requested
    viz_df = pivot_df_top.copy()
    if normalize_rows:
        row_sums = viz_df.sum(axis=1)
        viz_df = viz_df.div(row_sums, axis=0)
        fmt = ".2f" if normalize_rows else "g"
    else:
        fmt = "g"
    ...
    heatmap = sns.heatmap(viz_df, cmap=cmap, annot=False, linewidths=0.5, ax=ax)
    ...
    for i, row_idx in enumerate(viz_df.index):
        for j, col_idx in enumerate(viz_df.columns):
            ...
            if normalize_rows:
                # For normalized heatmaps - only mark significant cells with *
                if is_significant:
                    text = ax.text(
                        j + 0.5,
                        i + 0.5,
...
def create_heatmap_packing_week(
    ...
    value_col : str
        Column name to use for values/counts
    title : str, optional
        Title for the heatmap. If None, it will be generated based on col_col.
    figsize : tuple, optional
        Figure size (width, height)
    cmap : str, optional
        Colormap for the heatmap
    ...
    if title is None:
        title = f"Heatmap of Packing Week by {entity_name} (Container Count)"
    ...
    top_entities = col_totals.nlargest(top_n).index.tolist()
...
def calculate_concentration_metrics(df, entity1_col, entity2_col):
    """
...
def plot_concentration_bubble(
    ...
    cbar.set_label(f"% of pallets to top {entity2_col}")
    ...
    ax.set_ylabel("Number of Unique Containers", fontsize=12)
